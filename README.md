...
- CSV、完整工作区 JSON 导出。
- 仪表盘、审计日志和健康检查。
- 低余量与临期批次提醒：按时区与每日时刻生成批次级事件，规则变更只重算未发项，重复扫描不重复通知。
...
- 归档代替核心数据硬删除。
- 审计日志只追加，不更新、不删除。
- 提醒按批次生成事件（`LOW_STOCK` 每批次一轮、`EXPIRY` 每个提前天数档位一轮），以操作员时区的本地日历日判定触发日。
- 提醒重算只动未发项：未发项可取消/改期，已发事件目标消失才标记解除，绝不重发；部分唯一索引 + 行锁保证重复扫描/多实例不重复通知。
